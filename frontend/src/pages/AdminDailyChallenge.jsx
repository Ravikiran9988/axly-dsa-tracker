      ...
      <div key={c.id} className="p-4 flex flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="font-bold text-theme-text1 hover:text-amber-400 transition-colors cursor-pointer truncate" onClick={() => setPreviewChallenge(c)}>{c.title}</div>
        ...
        <div className="flex items-center justify-end gap-1.5 pt-2 border-t border-theme-border">
          <button onClick={() => setPreviewChallenge(c)} className="p-1.5 rounded-lg text-theme-text2 hover:text-theme-text1 hover:bg-theme-surface2 transition-colors" title="View Details"><Eye className="w-3.5 h-3.5" /></button>
          {c.status !== 'archived' && <button onClick={() => setEditingChallenge(c)} className="p-1.5 rounded-lg text-theme-text2 hover:text-amber-400 hover:bg-theme-surface2 transition-colors" title="Edit Challenge"><Edit2 className="w-3.5 h-3.5" /></button>}
          {c.status !== 'archived' && <button onClick={() => setSchedulingChallenge(c)} className="p-1.5 rounded-lg text-theme-text2 hover:text-cyan-400 hover:bg-theme-surface2 transition-colors" title="Schedule Date"><Calendar className="w-3.5 h-3.5" /></button>}
          {c.status !== 'archived' && <button onClick={() => handleTogglePublish(c)} className={`p-1.5 rounded-lg transition-colors ${c.status === 'published' ? 'text-emerald-400 hover:text-amber-400 hover:bg-theme-surface2' : 'text-theme-text2 hover:text-emerald-400 hover:bg-theme-surface2'}`} title={c.status === 'published' ? 'Unpublish' : 'Publish'}>{c.status === 'published' ? <Check className="w-3.5 h-3.5" /> : <Send className="w-3.5 h-3.5" />}</button>}
          {c.status !== 'archived' && c.status !== 'published' && <button onClick={() => handlePublishNow(c)} className="p-1.5 rounded-lg text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 transition-colors" title="Publish Now (Make Today's Active Challenge)"><Zap className="w-3.5 h-3.5" /></button>}
          {c.status !== 'archived' && <button onClick={() => handleArchive(c.id)} className="p-1.5 rounded-lg text-theme-text3 hover:text-theme-text2 hover:bg-theme-surface2 transition-colors" title="Archive Challenge"><Archive className="w-3.5 h-3.5" /></button>}
          <button onClick={() => setDeletingChallenge(c)} className="p-1.5 rounded-lg text-theme-text3 hover:text-rose-400 hover:bg-theme-surface2 transition-colors" title="Delete Challenge"><Trash2 className="w-3.5 h-3.5" /></button>
        </div>
      </div>
    ))
  )}
