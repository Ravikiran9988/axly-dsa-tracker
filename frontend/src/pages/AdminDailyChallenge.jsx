  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5">
        {[
          ['Total Challenges', stats.total, 'text-white', 'border-theme-border'],
          ...
          ['Archived', stats.archived, 'text-theme-text3', 'border-theme-border']
      ...
      <div className="p-4 rounded-2xl bg-theme-surface border border-theme-border flex flex-wrap items-center justify-between gap-3"><form onSubmit={handleSearchSubmit} className="flex items-center gap-2 flex-1 min-w-[240px]"><div className="relative flex-1"><Search className="w-3.5 h-3.5 text-theme-text2 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" placeholder="Search challenges by title or topic..." value={search} onChange={(e) => setSearch(e.target.value)} className="input-field pl-8 w-full text-xs" /></div><button type="submit" className="btn-secondary btn-sm text-xs">Search</button></form><div className="flex flex-wrap items-center gap-2 text-xs"><select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="input-field py-1.5 text-xs font-mono"><option value="">All Difficulties</option><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></select><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="input-field py-1.5 text-xs font-mono"><option value="">All Statuses</option><option value="draft">Draft</option><option value="scheduled">Scheduled</option><option value="published">Published</option><option value="archived">Archived</option></select><button onClick={() => { setSearch(''); setDifficulty(''); setTopicId(''); setStatusFilter(''); setDateFilter(''); }} className="p-2 rounded-xl text-theme-text2 hover:text-white hover:bg-theme-surface2 transition-colors" title="Reset filters"><RefreshCw className="w-3.5 h-3.5" /></button></div></div>

      <div className="card overflow-hidden border border-theme-border"><div className="px-6 py-4 border-b border-theme-border flex items-center justify-between bg-theme-surface"><div><h3 className="text-sm font-bold text-white">Daily Challenge Repository</h3><p className="text-[11px] text-theme-text2">All authored competitive challenges, scheduled dates, and creation sources</p></div><span className="text-xs text-theme-text2 font-mono">{challenges.length} Problem{challenges.length !== 1 ? 's' : ''}</span></div><div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr className="border-b border-theme-border bg-theme-surface text-theme-text2 font-mono text-[11px] uppercase"><th className="py-3.5 px-4 font-semibold">Title</th><th className="py-3.5 px-3 font-semibold">Difficulty</th><th className="py-3.5 px-3 font-semibold">Topic</th><th className="py-3.5 px-3 font-semibold">Pattern</th><th className="py-3.5 px-3 font-semibold">Points</th><th className="py-3.5 px-3 font-semibold">Status</th><th className="py-3.5 px-3 font-semibold">Scheduled Date</th><th className="py-3.5 px-3 font-semibold">Created Via</th><th className="py-3.5 px-4 font-semibold text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-800/60 font-mono">{loading ? Array.from({ length: 5 }).map((_, idx) => <tr key={idx} className="animate-pulse"><td colSpan={9} className="py-4 px-4 text-center text-theme-text3">Loading daily challenges...</td></tr>) : challenges.length === 0 ? <tr><td colSpan={9} className="py-8 text-center text-theme-text3 font-sans">No Daily Challenges found.</td></tr> : challenges.map((c) => <tr key={c.id} className="hover:bg-theme-surface transition-colors"><td className="py-3 px-4"><div className="font-sans font-bold text-theme-text1 hover:text-amber-400 transition-colors cursor-pointer" onClick={() => setPreviewChallenge(c)}>{c.title}</div><div className="text-[10px] text-theme-text3 font-mono truncate max-w-xs">{c.slug || c.id}</div></td><td className="py-3 px-3"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${difficultyColors[c.difficulty?.toLowerCase()] || ''}`}>{c.difficulty}</span></td><td className="py-3 px-3 text-theme-text2 font-sans">{c.topic_name || 'Arrays'}</td><td className="py-3 px-3 text-theme-text2 text-[11px] font-sans">{c.pattern_name || c.pattern || '—'}</td><td className="py-3 px-3 font-bold text-amber-400">+{c.points || 100}</td><td className="py-3 px-3"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${statusColors[c.status?.toLowerCase()] || ''}`}>{c.status}</span></td><td className="py-3 px-3 text-theme-text2">{c.scheduled_date ? <span className="text-cyan-400 flex items-center gap-1"><Calendar className="w-3 h-3" /> {c.scheduled_date}</span> : <span className="text-slate-600">—</span>}</td><td className="py-3 px-3">{c.created_via === 'ai' ? <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-500/15 text-purple-300 border border-purple-500/30 inline-flex items-center gap-1"><Sparkles className="w-2.5 h-2.5" /> AI</span> : <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-theme-surface2 text-theme-text2 border border-theme-border inline-flex items-center gap-1">✍ Manual</span>}</td><td className="py-3 px-4 text-right"><div className="flex items-center justify-end gap-1.5"><button onClick={() => setPreviewChallenge(c)} className="p-1.5 rounded-lg text-theme-text2 hover:text-white hover:bg-theme-surface2 transition-colors" title="View Details"><Eye className="w-3.5 h-3.5" /></button>{c.status !== 'archived' && <button onClick={() => setEditingChallenge(c)} className="p-1.5 rounded-lg text-theme-text2 hover:text-amber-400 hover:bg-theme-surface2 transition-colors" title="Edit Challenge"><Edit2 className="w-3.5 h-3.5" /></button>}{c.status !== 'archived' && <button onClick={() => setSchedulingChallenge(c)} className="p-1.5 rounded-lg text-theme-text2 hover:text-cyan-400 hover:bg-theme-surface2 transition-colors" title="Schedule Date"><Calendar className="w-3.5 h-3.5" /></button>}{c.status !== 'archived' && <button onClick={() => handleTogglePublish(c)} className={`p-1.5 rounded-lg transition-colors ${c.status === 'published' ? 'text-emerald-400 hover:text-amber-400 hover:bg-theme-surface2' : 'text-theme-text2 hover:text-emerald-400 hover:bg-theme-surface2'}`} title={c.status === 'published' ? 'Unpublish' : 'Publish'}>{c.status === 'published' ? <Check className="w-3.5 h-3.5" /> : <Send className="w-3.5 h-3.5" />}</button>}{c.status !== 'archived' && c.status !== 'published' && <button onClick={() => handlePublishNow(c)} className="p-1.5 rounded-lg text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 transition-colors" title="Publish Now (Make Today's Active Challenge)"><Zap className="w-3.5 h-3.5" /></button>}{c.status !== 'archived' && <button onClick={() => handleArchive(c.id)} className="p-1.5 rounded-lg text-theme-text3 hover:text-theme-text2 hover:bg-theme-surface2 transition-colors" title="Archive Challenge"><Archive className="w-3.5 h-3.5" /></button>}<button onClick={() => setDeletingChallenge(c)} className="p-1.5 rounded-lg text-theme-text3 hover:text-rose-400 hover:bg-theme-surface2 transition-colors" title="Delete Challenge"><Trash2 className="w-3.5 h-3.5" /></button></div></td></tr>)}</tbody></table></div></div>
      ...
      {previewChallenge && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in"><div className="bg-theme-surface border border-theme-border rounded-3xl w-full max-w-2xl max-h-[85vh] overflow-y-auto custom-scrollbar p-6 space-y-4 shadow-2xl"><div className="flex items-center justify-between pb-3 border-b border-theme-border"><div className="flex items-center gap-2"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${difficultyColors[previewChallenge.difficulty?.toLowerCase()] || ''}`}>{previewChallenge.difficulty}</span><h3 className="text-base font-bold text-white">{previewChallenge.title}</h3></div><button onClick={() => setPreviewChallenge(null)} className="text-theme-text2 hover:text-white"><X className="w-4 h-4" /></button></div><div className="space-y-3 text-xs"><div className="p-3 rounded-xl bg-theme-surface border border-theme-border"><span className="text-[11px] font-mono font-bold text-theme-text2 uppercase">Problem Description</span><p className="text-theme-text1 whitespace-pre-line mt-1">{previewChallenge.description}</p></div>{previewChallenge.constraints && <div className="p-3 rounded-xl bg-theme-surface border border-theme-border"><span className="text-[11px] font-mono font-bold text-theme-text2 uppercase">Constraints</span><p className="text-theme-text2 font-mono text-[11px] mt-1 whitespace-pre-line">{previewChallenge.constraints}</p></div>}{previewChallenge.editorial && <div className="p-3 rounded-xl bg-theme-surface border border-theme-border"><span className="text-[11px] font-mono font-bold text-amber-400 uppercase">Editorial / Solution Approach</span><p className="text-theme-text2 mt-1 whitespace-pre-line">{previewChallenge.editorial}</p></div>}</div><div className="flex justify-end gap-2 pt-2 border-t border-theme-border"><button onClick={() => setPreviewChallenge(null)} className="btn-secondary btn-sm text-xs">Close</button></div></div></div>}
      {showLogsModal && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in"><div className="bg-theme-surface border border-theme-border rounded-3xl w-full max-w-3xl max-h-[85vh] overflow-y-auto custom-scrollbar p-6 space-y-4 shadow-2xl"><div className="flex items-center justify-between pb-3 border-b border-theme-border"><div className="flex items-center gap-2"><Bot className="w-5 h-5 text-purple-400" /><h3 className="text-base font-bold text-white">Daily Challenge Automation Logs</h3></div><button onClick={() => setShowLogsModal(false)} className="text-theme-text2 hover:text-white"><X className="w-4 h-4" /></button></div><div className="space-y-2">{automationLogs.length === 0 ? <div className="py-8 text-center text-theme-text3 text-xs">No automation run logs found.</div> : automationLogs.map((log) => <div key={log.id} className="p-3 rounded-xl bg-theme-surface border border-theme-border text-xs flex items-center justify-between"><div className="space-y-1"><div className="flex items-center gap-2"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${log.status === 'success' ? 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' : log.status === 'failed' ? 'text-rose-400 bg-rose-500/10 border border-rose-500/20' : 'text-theme-text2 bg-theme-surface2'}`}>{log.status}</span><span className="font-mono text-cyan-400 font-bold">Target: {log.target_date}</span><span className="text-theme-text3">Mode: {log.mode}</span>{log.attempt_count > 0 && <span className="text-theme-text2">Attempts: {log.attempt_count}</span>}</div><p className="text-[11px] text-theme-text2">{log.details}</p></div><div className="text-[10px] text-theme-text3 font-mono shrink-0">{new Date(log.created_at).toLocaleString()}</div></div>)}</div><div className="flex justify-end pt-2 border-t border-theme-border"><button onClick={() => setShowLogsModal(false)} className="btn-secondary btn-sm text-xs">Close</button></div></div></div>}
      {deletingChallenge && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in"><div className="bg-theme-surface border border-rose-500/30 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl"><div className="flex items-center gap-3 text-rose-400"><div className="p-2.5 rounded-2xl bg-rose-500/10 border border-rose-500/20"><Trash2 className="w-5 h-5" /></div><div><h3 className="text-sm font-bold text-white">Delete Daily Challenge?</h3><p className="text-[11px] text-theme-text2">This action will permanently delete the problem and its test cases.</p></div></div><div className="p-3.5 rounded-xl bg-theme-surface border border-theme-border text-xs"><strong className="text-white">{deletingChallenge.title}</strong><div className="text-theme-text3 text-[11px] mt-0.5">{deletingChallenge.id} &middot; {deletingChallenge.difficulty}</div></div><div className="flex items-center justify-end gap-2 pt-2"><button onClick={() => setDeletingChallenge(null)} className="btn-secondary btn-sm text-xs">Cancel</button><button onClick={handleDeleteConfirm} className="btn-primary btn-sm text-xs bg-rose-600 hover:bg-rose-500 text-white font-bold">Confirm Delete</button></div></div></div>}
    </div>
  );
}
