    ...
    <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-cyan-400 font-mono">@{profileData.username || 'student'}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3.5">
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
          <div className="text-xl font-bold text-white">{stats.total_challenges}</div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
          <div className="text-xl font-bold text-cyan-400">{stats.accuracy_rate}</div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
          <div className="text-xl font-bold text-amber-400 flex items-center justify-center gap-1">
            <Flame className="w-4 h-4 fill-amber-400" /> {stats.streak}d
          </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center col-span-2 sm:col-span-4 lg:col-span-1">
          <div className="text-xl font-bold text-indigo-400">{stats.points} pts</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-4">
          ...
          <div className="space-y-2.5">
            {recent_submissions && recent_submissions.length > 0 ? (
              ...
              <div className="text-xs text-slate-500">No recent submissions.</div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-4">
          <h2 className="text-sm font-bold text-white tracking-wider uppercase flex items-center gap-2">
            ...
            <span>Recent Mentor Reviews</span>
          </h2>
          <div className="space-y-2.5">
            {recent_feedback && recent_feedback.length > 0 ? (
              recent_feedback.map(fb => (
                <div key={fb.id} className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-white">{fb.question_title}</span>
                    <span className="text-[10px] font-semibold text-orange-400 uppercase">{fb.review_status}</span>
                  </div>
                  <p className="text-slate-300 text-[11px] leading-relaxed">"{fb.feedback}"</p>
                </div>
              ))
            ) : (
              <div className="text-xs text-slate-500">No mentor feedback records yet.</div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-lg rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl p-6 space-y-5">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-3.5 text-xs">
              ...
              <div>
                <label className="block text-slate-400 font-semibold mb-1">Skills (comma separated)</label>
                <input
                  type="text"
                  value={skillsInput}
                  onChange={(e) => setSkillsInput(e.target.value)}
                  placeholder="JavaScript, Python, Algorithms, React"
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-white focus:outline-none focus:border-cyan-500"
                />
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 flex justify-end gap-2.5">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold flex items-center gap-1.5 shadow-md shadow-cyan-950"
                >
