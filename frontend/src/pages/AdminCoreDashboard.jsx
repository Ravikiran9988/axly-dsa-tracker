...
export default function AdminCoreDashboard({ onSelectProblem }) {
  ...
  return <div className="max-w-7xl mx-auto space-y-6">
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4"><div><div className="flex items-center gap-2"><h1 className="text-3xl font-extrabold text-white">Admin</h1><span className="text-[10px] px-2 py-1 rounded-full bg-amber-500/10 text-amber-300 border border-amber-500/20 font-bold">CORE</span></div><p className="text-sm text-slate-400 mt-1">Manage the question library and the global Daily Challenge.</p></div><div className="flex gap-2"><button onClick={() => setDailyModal(true)} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-sm font-bold text-white"><Sparkles className="w-4 h-4" /> Daily Question</button><button onClick={() => { setEditing(null); setQuestionModal(true); }} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-sm font-bold text-white"><Plus className="w-4 h-4" /> Add Question</button></div></div>
    {error && <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-sm">{error}</div>}
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">{[
      ['Students', stats?.total_users ?? 0, Users], ['Active Questions', stats?.total_active_questions ?? 0, Layers], ['Solved', stats?.total_solved_submissions ?? 0, CheckCircle2]
    ].map(([label,value,Icon]) => <div key={label} className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800"><div className="flex justify-between"><span className="text-xs uppercase tracking-wider text-slate-400 font-bold">{label}</span><Icon className="w-4 h-4 text-cyan-400" /></div><div className="mt-3 text-2xl font-extrabold text-white">{value}</div></div>)}</div>
