...
export default function StudentProfile({ onSelectProblem }) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-theme-surface2 border border-cyan-900/30 shadow-2xl relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <h1 className="text-xl md:text-2xl font-extrabold text-theme-text1 tracking-tight">
                  {profileData.name}
                </h1>
              ...
              <p className="text-xs text-cyan-500 dark:text-cyan-400 font-mono">@{profileData.username || profileData.email?.split('@')[0] || 'student'}</p>
              <div className="flex flex-wrap items-center gap-4 text-xs text-theme-text2 pt-1">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-theme-surface2 hover:bg-theme-surface3 text-theme-text1 text-xs font-semibold border border-theme-border shadow-md transition-all active:scale-95"
          >
        ...
        {/* Bio & Links */}
        <div className="mt-6 pt-6 border-t border-theme-border grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
            ...
            {profileData.skills && profileData.skills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {profileData.skills.map((skill, idx) => (
                  <span key={idx} className="text-[11px] px-2.5 py-0.5 rounded-lg bg-cyan-950/20 text-cyan-600 dark:text-cyan-300 border border-cyan-800/20">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          <div className="md:col-span-4 flex items-center justify-start md:justify-end gap-3">
            {profileData.github_url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="p-2.5 rounded-xl bg-theme-surface border border-theme-border text-theme-text2 hover:text-theme-text1 hover:border-theme-border transition-colors"
                title="GitHub"
              >
