...
import { Play, CheckCircle2, AlertTriangle, RotateCcw, Code, Github, Zap, HelpCircle, FileCode, Check, Copy, ArrowLeft, Terminal as TerminalIcon, History } from 'lucide-react';
...
export default function ProblemWorkspace({ questionId, onBack, onStatusUpdated }) {
  ...
  const [language, setLanguage] = useState('javascript'), [sourceCode, setSourceCode] = useState('');
  const [submissionMethod, setSubmissionMethod] = useState('code'), [githubUrl, setGithubUrl] = useState('');
  const [isSubmittingGithub, setIsSubmittingGithub] = useState(false), [githubSuccessMessage, setGithubSuccessMessage] = useState(null);
  const [isRunning, setIsRunning] = useState(false), [isSubmitting, setIsSubmitting] = useState(false), [execResult, setExecResult] = useState(null);
  const [selectedTestCaseTab, setSelectedTestCaseTab] = useState(0), [customInput, setCustomInput] = useState('');
  const [leftTab, setLeftTab] = useState('description'), [bottomTab, setBottomTab] = useState('testcases');
  const [copied, setCopied] = useState(false), [pastSubmissions, setPastSubmissions] = useState([]);
  ...
  async function handleSubmitGithub(e){e.preventDefault();if(!githubUrl.trim())return;setIsSubmittingGithub(true);setGithubSuccessMessage(null);try{await api.submitChallenge({question_id:questionId,submission_type:'github',github_url:githubUrl.trim()});setGithubSuccessMessage('GitHub submission received! Your code is queued for mentor code review.');if(onStatusUpdated)onStatusUpdated();}catch(err){alert(err.message||'Failed to submit GitHub link.');}finally{setIsSubmittingGithub(false);}}
  if(loading)return <div className="flex-1 flex flex-col items-center justify-center min-h-[500px] text-slate-400"><div className="w-10 h-10 border-4 border-cyan-500/20 border-t-cyan-500 rounded-full animate-spin mb-4"/><div className="text-xs font-mono">Initializing Sandboxed Problem Environment...</div></div>;
