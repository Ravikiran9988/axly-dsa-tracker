...
function listSubmissions({ user, question_id, status, review_status, page = 1, limit = 50 }) {
  ...
  let whereClauses = [];
  const params = [];
  if (user.role !== 'admin' && user.role !== 'mentor') { whereClauses.push('s.user_id = ?'); params.push(user.id); }
  if (question_id) { whereClauses.push('s.question_id = ?'); params.push(question_id); }
  if (status) { whereClauses.push('s.status = ?'); params.push(status); }
  if (review_status) { whereClauses.push('s.review_status = ?'); params.push(review_status); }
  const whereSql = whereClauses.length ? `WHERE ${whereClauses.join(' AND ')}` : '';
  const total = db.prepare(`SELECT COUNT(*) as total FROM submissions s ${whereSql}`).get(...params)?.total || 0;
  const data = db.prepare(`
    SELECT s.*, q.title as question_title, q.difficulty as question_difficulty, q.points as question_points,
      t.name as topic_name, u.name as user_name, u.email as user_email, u.avatar_url as user_avatar,
      rev.name as reviewer_name
    FROM submissions s JOIN questions q ON s.question_id = q.id LEFT JOIN topics t ON q.topic_id = t.id
    JOIN users u ON s.user_id = u.id LEFT JOIN users rev ON s.reviewer_id = rev.id
    ${whereSql} ORDER BY s.updated_at DESC, s.created_at DESC LIMIT ? OFFSET ?
...
function reviewSubmission({ submission_id, reviewer_id, review_status, feedback }) {
  if (!['approved','changes_requested','rejected'].includes(review_status)) throw new AppError('review_status must be one of: approved, changes_requested, rejected',400,'VALIDATION_ERROR');
...
function updateSubmission({ submission_id, question_id, user_id, status }) {
  let submission = submission_id ? db.prepare('SELECT * FROM submissions WHERE id=?').get(submission_id) : question_id ? db.prepare('SELECT * FROM submissions WHERE user_id=? AND question_id=?').get(user_id,question_id) : null;
