...
async function reviewCode({ submission_id }) {
  const submission = db.prepare(`SELECT s.*, q.title, q.description, q.problem_statement, q.constraints
    FROM submissions s JOIN questions q ON q.id=s.question_id WHERE s.id=?`).get(submission_id);
  ...
  const response = await fetch(`${base}/chat/completions`, { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${apiKey}`}, body:JSON.stringify({model,messages:[{role:'system',content:'You are a rigorous DSA code reviewer. Be concise and factual.'},{role:'user',content:prompt}],temperature:0.1}) });
  ...
  db.prepare('UPDATE submissions SET ai_score=?,ai_feedback=?,ai_reviewed_at=?,final_score=COALESCE(manual_score,?) ,updated_at=? WHERE id=?').run(score,feedback,now,score,now,submission_id);
