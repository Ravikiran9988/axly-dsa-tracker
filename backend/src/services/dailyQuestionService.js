...
function getAutoDailyQuestion(date) {
  ...
  return questions[((epochDays % questions.length) + questions.length) % questions.length];
}
...
function getDailyQuestion(user, targetDate = null) {
  ...
  const scheduled = db.prepare(`
    SELECT dq.id AS daily_question_id, dq.date, dq.created_at AS scheduled_at,
           q.id, q.title, q.difficulty, q.topic_id, t.name AS topic_name,
           q.url, q.is_active, q.points
    FROM daily_questions dq
    JOIN questions q ON dq.question_id=q.id
    LEFT JOIN topics t ON q.topic_id=t.id
    WHERE dq.date=? AND q.is_active=1 AND q.status='published'
  `).get(date);
  ...
  const submission = db.prepare(`
    SELECT id, status, attempted_at, solved_at, final_score
