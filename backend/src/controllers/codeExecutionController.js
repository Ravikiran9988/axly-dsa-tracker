...
async function runCode(req, res, next) {
  try {
    ...
    if (custom_input !== undefined && custom_input !== null && custom_input.trim() !== '') {
      ...
      testCasesToRun = db.prepare(`SELECT id,input,expected_output,is_hidden FROM test_cases WHERE question_id=? AND is_hidden=0 ORDER BY created_at ASC`).all(question_id);
...
/** Submit against all tests and persist Phase 1 objective scoring. */
async function submitSolution(req, res, next) {
  try {
    ...
    // Record first/next attempt before updateSubmission so elapsed time is measured
    // from the assignment/start timestamp rather than the current request.
    recordAttempt({
      userId,
    ...
    updateSubmission({ question_id, user_id: userId, status: finalStatus });

    db.prepare(`UPDATE submissions SET language=?,source_code=?,passed_tests=?,total_tests=?,execution_time_ms=? WHERE user_id=? AND question_id=?`).run(
      language, source_code, execResult.passed_tests, allTestCases.length, execResult.execution_time_ms, userId, question_id
    );

    const logId = uuidv4();
    db.prepare(`INSERT INTO code_submissions_log (id,user_id,question_id,language,source_code,status,passed_tests,total_tests,execution_time_ms) VALUES (?,?,?,?,?,?,?,?,?)`).run(
      logId, userId, question_id, language, source_code, execResult.status, execResult.passed_tests, allTestCases.length, execResult.execution_time_ms
    );

    const progress = progressService.getUserProgress(userId);
    const updatedSubmission = db.prepare('SELECT * FROM submissions WHERE user_id=? AND question_id=?').get(userId, question_id);

    return res.status(200).json({ data: {
      submission_id: logId, question_id, language, status: execResult.status,
      passed_tests: execResult.passed_tests, total_tests: allTestCases.length,
      execution_time_ms: execResult.execution_time_ms, submission_status: finalStatus,
      results: execResult.results,
      scoring: {
        test_score: updatedSubmission.test_score,
        time_score: updatedSubmission.time_score,
        attempt_score: updatedSubmission.attempt_score,
        final_score: updatedSubmission.final_score,
        attempt_count: updatedSubmission.attempt_count,
        solve_duration_seconds: updatedSubmission.solve_duration_seconds,
        score_max: 100
      },
      progress
    } });
...
async function getSubmissionsHistory(req, res, next) {
  try {
    ...
    const userId = req.user.id;
    const history = db.prepare(`SELECT id,question_id,language,source_code,status,passed_tests,total_tests,execution_time_ms,created_at FROM code_submissions_log WHERE user_id=? AND question_id=? ORDER BY created_at DESC LIMIT 50`).all(userId, question_id);
